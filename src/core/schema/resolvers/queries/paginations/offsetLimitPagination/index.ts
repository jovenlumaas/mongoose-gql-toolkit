...
export const makeOffsetLimitPaginationResolverFn: TMakeOffsetLimitPaginationResolverFn<any> =
  (model, authValidations) => (options) => {
    const baseResolver = getBaseResolver({
      ...(options as TAuthValidatorOptions),
      authValidations,
    });
    return baseResolver.createResolver(async (_, { filters = {}, pageArgs }, { dataSources: { models } }) => {
