...
) => <TDocument = any>(options?: TMakeCountOptions<TDocument>) => TResolverFn<TContext>;

export const makeCountResolverFn: TMakeCountResolverFn = (model, authValidations) => (options) => {
  ...
  return baseResolver.createResolver(async (_, args, { dataSources: { models } }) => {
    // info.cacheControl.setCacheHint({ maxAge: 60, scope: "PRIVATE" });
    return await models[model].countDocuments({ ...args, ...where }, qryOpts);
  }) as any;
};
