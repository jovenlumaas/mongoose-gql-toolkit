...
> = (model: keyof TModels, authValidations: TAuthValidations) => (options?: TUpdateOptions) => TResolverFn<TContext>;

export const makeUpdateOneResolverFn: TMakeUpdateOneResolverFn<any> = (model, authValidations) => (options) => {
  const { isBoolean = false, manualSaveKeys = [], ...authOptions } = ({} = options ?? {});
  ...
  return baseResolver.createResolver(async (_, { _id, input }, { dataSources: { models } }) => {
    try {
