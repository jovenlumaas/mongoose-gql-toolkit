...
export const makeDeleteOneResolverFn: TMakeDeleteOneResolverFn<any> =
  (model, authValidations) => (options) => {
    ...
    return baseResolver.createResolver(
      async (_, { _id }, { dataSources: { models } }) => {
        try {
...
export const makeRestoreDeletedOneResolverFn: TMakeRestoreDeletedOneResolverFn<
  any
> = (model, authValidations) => (options) => {
  let baseResolver = getBaseResolver({
    ...(options as TAuthValidatorOptions),
  ...
  return baseResolver.createResolver(
    async (_, { id }, { dataSources: { models, sequelize } }) => {
      try {
