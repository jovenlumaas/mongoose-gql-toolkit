...
type TIncludeURLFn = <T extends Record<string, string>, U extends string>(
  tags: T,
  url: U
) => FetchTags<T>;
...
const includeURL: TIncludeURLFn = (tags, url) => {
  return Object.keys(tags).reduce(
    (acc, key) => ({ ...acc, [key]: { query: tags[key], url } }),
    {} as any
  );
};
...
const mergeFetchTags = <T extends Record<string, TFetchTag>>(
  tags: T
): TMergedFetchTags<T> => {
  let tagKeys: string[] = [];
  ...
  return Object.keys(tags).reduce((acc, key) => {
    if (tagKeys.includes(key)) {
      throw new TypeError(
        `Error on 'mergeFetchTags': key ${key} is already declared`
      );
    } else {
      tagKeys.push(key);
